/* ============================================================
   STORE.JS  —  Single source of truth for the portfolio.
   All content lives in the browser's localStorage so you can
   edit everything from the Admin Panel without touching code.
   ============================================================ */

(function (global) {
  "use strict";

  var STORAGE_KEY = "manzar_portfolio_data_v1";
  var AUTH_KEY = "manzar_portfolio_auth_v1";

  /* A clean default avatar (monogram) used until you upload a photo. */
  var DEFAULT_AVATAR =
  ...
  /* ---------- DEFAULT DATA (seeded from the résumé) ---------- */
  var DEFAULT_DATA = {
    profile: {
      name: "Manzar Abbas",
  ...
  function getData() {
    try {
      ...
      if (!raw) {
        var seed = deepClone(DEFAULT_DATA);
        localStorage.setItem(STORAGE_KEY, JSON.stringify(seed));
        return seed;
      }
      var parsed = JSON.parse(raw);
      // Make sure newly-added top-level keys always exist.
      Object.keys(DEFAULT_DATA).forEach(function (k) {
        if (parsed[k] === undefined) parsed[k] = deepClone(DEFAULT_DATA[k]);
      });
      // Backfill newly-added profile fields (e.g. github / resume) on older saves.
      if (parsed.profile) {
        Object.keys(DEFAULT_DATA.profile).forEach(function (k) {
          if (parsed.profile[k] === undefined) parsed.profile[k] = deepClone(DEFAULT_DATA.profile[k]);
        });
        // These two shipped empty in the first version — seed them if still blank.
        if (!parsed.profile.github) parsed.profile.github = DEFAULT_DATA.profile.github;
        if (!parsed.profile.resumeUrl) parsed.profile.resumeUrl = DEFAULT_DATA.profile.resumeUrl;
      }
      return parsed;
    } catch (e) {
      console.error("Store read failed, using defaults:", e);
  ...
  function saveData(data) {
    try {
      ...
      alert(
        "Could not save. The image may be too large for browser storage. " +
          "Try a smaller / compressed photo."
      );
      return false;
    }
  }
  ...
  /* ---------- Simple client-side auth (not real security) ---------- */
  function getPassword() {
    return localStorage.getItem(AUTH_KEY) || "ManzarAbbas@512";
  }
